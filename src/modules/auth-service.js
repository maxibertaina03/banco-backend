...
async function getOrCreateUser(clerkId) {
  const result = await pool.query(
    'SELECT u.*, p.nombre, p.apellido, p.email FROM usuarios u JOIN personas p ON u.persona_id = p.id WHERE u.clerk_id = $1',
    [clerkId]
  );
  ...
  throw new HttpError(404, 'Usuario no encontrado. Debes registrarte primero con tu persona.');
}
